...
# PATCHED: upstream used `sed 's/\.//'` to turn "1.00" into 100, but Hyprland
# 0.56.2+ emits `"scale": 1`, leaving the divisor at 1 and inflating every
# margin 100x (full-screen warped wlogout). Also wrong for any fractional
# scale: 1.5 parsed as 15. awk is format-independent.
hypr_scale=$(hyprctl -j monitors | jq -r '.[] | select(.focused==true) | .scale' | awk '{printf "%.0f", $1*100}')
