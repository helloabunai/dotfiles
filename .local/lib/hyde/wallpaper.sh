...
Wall_Cache() {
    ...
    if [ "${set_as_global}" == "true" ]; then
        ...
        "${scrDir}/awwwallcache.sh" -w "${wallList[setIndex]}" &>/dev/null
...
Wall_Select() {
    ...
    mon_scale=$(jq -r '.[] | select(.focused==true) | .scale' <<<"${mon_data}" | awk '{printf "%.0f", $1*100}')
...
while true; do
    ...
    -j | --json)
        ...
        ;;
    -S | --select)
        "${scrDir}/awwwallcache.sh" w &>/dev/null &
    ...
    -g | --get)
        ...
        ;;
    -b | --backend)
        # Set wallpaper backend to use (awww, hyprpaper, etc.)
