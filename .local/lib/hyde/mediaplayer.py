...
def esc(text) -> str:
    """Escape &, <, > for Pango. Unescaped metadata makes waybar render a blank label."""
...
def create_tooltip_text(
    ...
    if artist or track:
        tooltip += f'<span foreground="{track_color}"><b>{esc(track)}</b></span>\n<span foreground="{artist_color}"><i>{esc(artist)}</i></span>\n'
...
def format_artist_track(artist, track, playing, max_length):
    ...
    if track and not artist:
        ...
        output_text = f"{prefix}{prefix_separator}<b>{esc(track)}</b>"
        ...
        output_text = (
            f"{prefix}{prefix_separator}<i>{esc(artist)}</i>{artist_track_separator}<b>{esc(track)}</b>"
        )
    else:
